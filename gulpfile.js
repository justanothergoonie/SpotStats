...
const filesForHtmlCheck = ['../public/*.html', '!node_modules/*.*'];
...
gulp.task('js-compile', (done) => {
	return (
		gulp
			...
			.pipe(
				gulp.dest((file) => file.base.replace('/src', '/public/dist'))
			) // put the js files here.
			.pipe(
				gulpFn(function (file) {
					if (file.path.indexOf('.js.map') === -1) {
						console.log('JS generated: '.cyan);
						console.log(file.path.replace('/src', '/public/dist'));
					}
				})
			)
	);
...
gulp.task('image-compress', (done) => {
	...
	return gulp
		...
		.pipe(plumber())
		.pipe(changed((file) => file.base.replace('/src', '/public/dist')))
		.pipe(imagemin())
		.pipe(gulp.dest((file) => file.base.replace('/src', '/public/dist'))) // put the image files here.
		.pipe(browserSync.stream()) // tell browsersync to send over the changes
		.pipe(
			gulpFn(function (file) {
...
gulp.task(
	'default',
	gulp.series('welcome', 'build', 'make-cool-shit', 'version')
);
